...
- **Known environment gap found while testing signup (2026-09-22)**: this Supabase project has "Confirm email" auth enabled but no SMTP sending configured, so `signUp()` fails with "Error sending confirmation email" and no user is created. The owner needs to either disable email confirmation (Supabase dashboard → Authentication → Providers → Email) or configure a custom SMTP sender, before real email signups will work. Login for already-confirmed accounts (e.g. ones created via the sibling site) is unaffected — this only blocks *new* signups through this app's form.
