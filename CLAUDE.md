...
- **2026-09-22 addition — owner-provided wellness practices DB grounds concrete suggestions**: owner supplied `wellness_practices.json`/`.xlsx` (375 items: domain × category × tier — domains 나 자신/인간관계/연인관계·부부생활/회사·커리어/육아; categories 몸으로 움직이기/생각 전환하기/관계·대화로 풀기/나를 돌보기/도움 요청하기; tiers 가볍게 시작/꾸준히 이어가기/장기 습관), kept outside the repo like the main Excel knowledge base. Imported via `scripts/build_practices_sql.mjs <json path>` (regenerate, don't hand-edit) into a new `wellness_practices` table (migration `20260922001000`). `lib/rag/practicesSearch.ts` picks up to 5 candidates per message (bigram overlap on title/detail/category + a domain-keyword-guess bonus + a light bonus for "가볍게 시작" tier, no embeddings — same reasoning as `serviceSearch.ts`'s small-table approach) for routes `wellness`/`life_decision`/`clinical_distress` (not `clinical_diagnosis` — that route is about diagnosis/medication questions, not activity suggestions; not persona-question mode — that's character interpretation, not advice). `prompt.ts` includes them as reference material the model should prefer over inventing generic suggestions, but never as a prescription. Falls back to no suggestions (not an error) if the migration hasn't been run yet.
