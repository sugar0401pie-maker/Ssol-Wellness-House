...
export function buildSystemPrompt(params: {
  ...
  // 2026-09-24 결정: 턴 수에 따라 3단계로 나눈다.
  //  1턴째(첫 답변): 듣고 반영만 (별도 지침 없음, 기존 Reflect 중심 동작)
  //  2턴째: 대략적인 방향(일반적인 수준)을 먼저 제안하고, 더 구체적인 걸 원하는지 확인
  //  3턴째부터: 더 묻지 말고 무조건 실질적이고 구체적으로 (여러 감정이 섞여 있으면
  ...
  if ((params.turnCount ?? 0) === 1) {
    parts.push(
      "이번이 이 대화의 2번째 답변입니다. 아직 완전히 구체화하지 않아도 되니, 지금까지 들은 내용을 바탕으로 대략적인 방향이나 일반적인 수준의 도움말을 먼저 이야기하세요. 그런 다음 더 자세하고 구체적인 것을 원하는지 물어보며 마무리하세요.",
    );
  }
  ...
  // "공감 표현"만 반복하지 말라는 지적 — 이건 턴 수와 무관하게 2번째 답변부터 계속 적용.
  if ((params.turnCount ?? 0) >= 1) {
    parts.push(
