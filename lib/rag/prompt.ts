...
export function buildSystemPrompt(params: {
  ...
  // 2026-09-24 결정: 기존엔 turnCount>=3(즉 4번째 답변부터)에야 구체화를 멈추고 제안으로
  // 넘어갔는데, "1~2회 정도만 구체화하고 이후부터는 구체적인 답변을 달라"는 피드백으로
  // 기준을 turnCount>=2(3번째 답변부터)로 한 턴 앞당겼다. 즉: 1번째 답변은 듣고 반영,
  // 2번째 답변까지는 필요하면 가볍게 더 물어봐도 되지만, 3번째 답변부터는 무조건 구체적인
  // 제안으로 넘어가야 한다.
  if ((params.turnCount ?? 0) >= 2) {
    parts.push(
      "이미 이 대화에서 1~2차례 들었습니다. 더 이상 새로운 확인 질문만 반복하지 말고, 지금까지 들은 내용을 바탕으로 '지금 상황은 이런 것 같아요' 식으로 짧게 정리한 뒤, 지금 바로 해볼 수 있는 단기적인 것과 꾸준히 이어가면 좋을 중장기적인 것을 구분해서 총 3가지 안팎으로 구체적으로 제안하세요. 마지막에는 새 질문 하나 대신, 추가로 궁금하거나 다르게 다루고 싶은 부분이 있는지 물어보며 마무리하세요.",
    );
  }
  ...
  // 하나씩 확인만 받으려 하지 말고, 여러 감정/어려움이 섞여 있으면 그 각각에 실질적인
  // 해결책을 바로 짝지어서 한 번에 다 제시하라는 지적(대부분의 고민은 여러 감정이 복합적으로
  // 섞여 있으니, 하나씩 순서대로 확인하는 게 아니라 한꺼번에 다뤄도 된다는 게 핵심).
